...
# NevoFlux: Also filter out -Xclang when its argument is being filtered, to avoid
# orphaned -Xclang consuming the next argument (e.g. the input file path).
def filter_preprocessor(cmd):
    ...
    while i < len(cmd):
        ...
        if arg.startswith(("-std=", "-std:")):
            i += 1
            continue
        # If this is -Xclang and the next arg would be filtered, skip both
        if (
            arg == "-Xclang"
            and i + 1 < len(cmd)
            and cmd[i + 1].startswith(("-std=", "-std:"))
        ):
            i += 2
            continue
        yield arg
    ...
    while True:
        ...
        if input.lower().endswith(".idl"):
            try:
                ...
                pass
        # NevoFlux: Use "--" to separate options from file paths to prevent
        # clang-cl (MSVC mode) from misinterpreting Unix absolute paths
        # starting with "/" as MSVC option flags (e.g. /workspace/... as /w).
